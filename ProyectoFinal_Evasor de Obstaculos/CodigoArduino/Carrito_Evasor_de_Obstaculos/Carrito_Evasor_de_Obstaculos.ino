...
Servo myservo;      // Variable correspondiente al servo motor

int Echo = A1;  //Pines del sensor ultra sónico.
int Trig = A0;
 
#define ENA 13// Pines de control del puente H.
#define ENB 12
#define IN1 46
#define IN2 48
#define IN3 50
 ...
 **********************/
const int InputNodes = 3; // incluye neurona de BIAS, correspondiente a las neuronas de entrada
const int HiddenNodes = 4; //incluye neurona de BIAS, Correespondiente a las capas ocultas
const int OutputNodes = 4;//Nodos de salida.
int i, j;
double Accum;
double Hidden[HiddenNodes];//Se almacena el valor de las capas para ser usado en...
double Output[OutputNodes];//Similar al caso enterior pero para las salidas.
//pesos correspondientes a la capa oculta
...
//Pesos correspondientes a la salida.
...
int Distance_test() 
{
  ...
  return (int)Fdistance;// Devuleve el valor de la distanc como un dato de tipo entero.
}

void setup() {
  myservo.attach(3);  // Pin de lectura del servomotor.
  Serial.begin(9600);
  /*=================================================================================================================================================
  ...
  ===================================================================================================================================================
  =================================================================================================================================================*/
  stop();// El pute H está desactvado
  myservo.write(90);  //posicion inicial del cervo a 90°
  delay(2000); 
} 

unsigned long previousMillis = 0;   // bace para medir ciclos de tiempo
const long interval =12;           // intervalos cada x milisegundos 12° de giro del servo para la detección de un obstáculo.
int grados_servo = 90;                // posicion del servo que mueve el sensor ultrasonico
bool clockwise = true;              // sentido de giro del servo(Horario)
const long ANGULO_MIN = 0; 
const long ANGULO_MAX = 180; 
double ditanciaMaxima = 60.0;       // distancia de lejania desde la que empieza a actuar la NN 
int incrementos =4;              // incrementos por ciclo de posicion del servo estaba en 9 va de 9° en 9° 
...
void conducir()
{
  ...
  **********************/
    double distance = double(Distance_test());// retoma el valor de l afuncion de prueba
    distance= double(constrain(distance, 0.0, ditanciaMaxima));//Restringe un número para que esté dentro de un rango.
  ...
  **********************/
    entrada2 = map(grados_servo, ANGULO_MIN, ANGULO_MAX, -100, 100);//Vuelve a mapear un número de un rango a otro
    entrada2 = double(constrain(entrada2, -100.00, 100.00));//Restringe un número para que esté dentro de un rango.
  ...
  **********************/
  ...
  //llamamos a la función InputToOutput, donde se evaluan las entradas con los pesos.
  InputToOutput(TestInput[0], TestInput[1], TestInput[2]); //ENTRADAS a ANN para obtener las SALIDAS.
